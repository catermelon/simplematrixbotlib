        ...
        try:
            if isinstance(event, KeyVerificationStart
                ...
                if "emoji" not in event.short_authentication_string:
                    logger.error("Other device does not support emoji verification "
                ...
                if isinstance(resp, ToDeviceError):
                    logger.debug(f"to_device failed with {resp}")
                ...
                logger.info(f"Verification has been cancelled by {event.sender} "
                ...
                logger.debug(f"{sas.get_emoji()}")

                yn = input("Do the emojis match? (Y/N) (C for Cancel) ")
                if yn.lower() == "y":
                    logger.debug("Match! The verification for this "
                    ...
                    logger.info("No match! Device will NOT be verified "
                    ...
                    logger.info("Cancelled by user! Verification will be "
                          "cancelled.")
                    resp = await self.async_client.cancel_key_verification(
                ...
                try:
                    ...
                    logger.error(f"Cancelled or protocol error: Reason: {e}.\n"
                    ...
                    logger.debug(f"sas.we_started_it = {sas.we_started_it}\n"
                ...
                logger.info(f"Received unexpected event type {type(event)}. "
